...
export interface ICompanyDocument extends Document {
    ...
    reviewedAt?: Date;
    // true = ban nop hien hanh cho cap (companyId, documentTypeId); khi dai
    // dien cong ty nop lai, ban cu duoc chuyen active=false (KHONG xoa) de giu
    // lich su - giong BusinessDocument.
    active: boolean;
    createdAt: Date;
    updatedAt: Date;
}
...
CompanyDocumentSchema.index({ companyId: 1, documentTypeId: 1, active: 1 });

export default (mongoose.models.CompanyDocument as Model<ICompanyDocument>) ||
